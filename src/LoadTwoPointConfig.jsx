...
const LoadTwoPointConfig = () => {
  ...
  const loadAvailableConfigs = async () => {
    try {
      ...
      const configs = await window.api.read2PointConfigs();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6">
      <div className="w-full mx-auto">
        <div className="flex items-center justify-start mb-8">
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Load 2-Point Configuration</h1>
        </div>
                          ...
                          <div>
                            <p className="font-medium text-slate-800">{config.configName}</p>
                             <p className="text-sm text-slate-500 mt-1">
                               {config.catheterToLoadCellDistance ? `${config.catheterToLoadCellDistance}mm, ` : ''}{config.forceLimit}gf, {config.probeTravelLimit}mm
                             </p>
                          </div>
                        </button>
                      ))
...
export default LoadTwoPointConfig;
